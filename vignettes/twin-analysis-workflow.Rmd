...
1. Apply univariate and multivariate (correlated factors) ACE and ADE models. The models include the output tables with parameters that we usually report.

2. Apply a cross-lag ACE model.
...
To ensure that the model fits the data well, we compare it against two reference models: saturated model and independence model. In a saturated model, there are as many parameters as there are data points (in terms of variances, covariances and means), therefore this models has the best fit. An independence model assumes that all variables are independent. Theoretically, a good model should describe the data as well as its saturated model. The function


To ensure that the model fits the data well, we compare it against two reference models: [saturated model](https://stats.stackexchange.com/questions/283/what-is-a-saturated-model) and independence model. 
